...
router.post('/subscribe', async (req, res, next) => {
  ...
  const output = `
        <hr>
        <center><a href="https://dreamworlld.herokuapp.com/"><img src="https://firebasestorage.googleapis.com/v0/b/mydocs-9999.appspot.com/o/Untitled-1.jpg?alt=media&token=242317b3-5147-4226-88fc-72aad455c0aa"></a></center>
        <hr>
    `;
...
// Update Order Page Route
router.post('/updateOrderPage/:id', async (req, res) => {
  const id = req.params.id;
  const order = await Orders.find({ _id: id });
  const AdminUpdatePanel = true;
...
router.post('/updateProduct/:id', async (req, res) => {
  ...
  const AdminUpdatePanel = true;
  const allProduct = await Product.find({ _id: id });
...
router.get('/checkout', isLoggedIn, async (req, res, next) => {
  ...
  var user = await Users.findOne({ _id: req.user.id });
  var cart = new Cart(req.session.cart);
